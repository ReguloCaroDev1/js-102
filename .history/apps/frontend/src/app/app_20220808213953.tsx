...
export function App() {
  ...
  const addTareaSchema = Yup.object().shape({
    tarea: Yup.string()
      .min(10, 'Too Short!')
      .max(100, 'Too Long!')
      .required('Required'),
  ...
  const nuevaTarea = useFormik({
    initialValues: {
      tarea: '',
      descripcion: '',
      status: 'Pending',
    },
    validationSchema: addTareaSchema,
    onSubmit: (values) => {
      console.log(values);
      /* await axios.post('http://localhost:3333/api/datos', values); */
      ...
      {datos ? (
        <>
          ...
          <div>
            {showModalAdd ? (
              <div>
                <form onSubmit={nuevaTarea.handleSubmit}>
                  <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                    <div className="relative w-auto my-6 mx-auto max-w-3xl">
                      <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                        ...
                        <div className="relative p-6 flex-auto">
                          <input
                            ...
                            onChange={nuevaTarea.handleChange('tarea')}
                            value={nuevaTarea.values.tarea}
                          />
                          <input
                            id="descripcion"
                            name="descripcion"
                            ...
                            onChange={nuevaTarea.handleChange('descripcion')}
                            value={nuevaTarea.values.descripcion}
                          />
                          ...
                          <select
                            id="status"
                            name="status"
                            onChange={nuevaTarea.handleChange('status')}
                            value={nuevaTarea.values.status}
